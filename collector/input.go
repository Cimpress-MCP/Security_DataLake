...
func (s *TcpServer) Start(f func() chan<- string) error {
	...
	go func() {
		for {
			...
			log.Infof("accepted new connection from %s", conn.RemoteAddr().String())
	...
	for {
		...
		if err != nil {
			if neterr, ok := err.(net.Error); ok && neterr.Timeout() {
				...
				log.Infof("Error from connection: %s", err)
				return
			}
		} else {
...
func (s *UdpServer) Start(f func() chan<- string) error {
	...
	if err != nil {
		log.Fatalf("failed to start UDP server", err)
		return err
	}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				log.Fatalf("failed to read UDP", err)
			}
			s.eventsRx.Inc(1)
